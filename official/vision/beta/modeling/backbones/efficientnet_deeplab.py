...
class DilatedEfficientNet(tf.keras.Model):
  """Class to build EfficientNet family model with Deeplabv3 modifications."""
  ...
  def __init__(self,
               ...
               norm_epsilon=0.001,
               **kwargs):
    """EfficientNet with DeepLabv3 modification initialization function.
  ...
  def _block_group(self, inputs, specs, name='block_group'):
    ...
    x = block_fn(
        ...
        dilation_rate=specs.dilation_rate,
...
@factory.register_backbone_builder('dilated_efficientnet')
def build_dilated_efficientnet(
  ...
  assert backbone_type == 'dilated_efficientnet', (f'Inconsistent backbone type '
                                           f'{backbone_type}')

  return DilatedEfficientNet(
