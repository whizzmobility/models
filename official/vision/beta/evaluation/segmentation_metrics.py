...
class Accuracy(tf.keras.metrics.Accuracy):
  ...
  This class utilizes tf.keras.metrics.Accuracy to perform batched accuracy when
  both input images and groundtruth masks are resized to the same size
  (rescale_predictions=False). It also computes accuracy on groundtruth original
  sizes, in which case, each prediction is rescaled back to the original image
  size.
  """
  ...
  def update_state(self, y_true, y_pred):
    ...
    super(Accuracy, self).update_state(
        flatten_masks, flatten_predictions,
        tf.cast(flatten_valid_masks, tf.float32))
